...
def _reduce_stat_scores(
    ...
    numerator = torch.where(
        zero_div_mask,
        torch.tensor(zero_division, dtype=numerator.dtype, device=numerator.device),
        numerator,
    )
    denominator = torch.where(
        zero_div_mask | ignore_mask,
        torch.tensor(1.0, dtype=denominator.dtype, device=denominator.device),
        denominator,
    )
    weights = torch.where(
        ignore_mask,
        torch.tensor(0.0, dtype=weights.dtype, device=weights.device),
        weights,
    )
    ...
    scores = torch.where(
        torch.isnan(scores),
        torch.tensor(zero_division, dtype=scores.dtype, device=scores.device),
        scores,
    )
    ...
    if average in (AverageMethod.NONE, None):
        return torch.where(
            ignore_mask, torch.tensor(float("nan"), device=scores.device), scores
        )
    return scores.sum()
